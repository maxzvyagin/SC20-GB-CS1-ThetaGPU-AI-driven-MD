...
from data import train_input_fn, val_input_fn, simulation_tf_record_input_fn, simulation_tf_record_eval_input_fn
...
def parse_args():
    ...
    parser.add_argument(
        "--cs-ip",
        default=None,
        help="CS-1 IP address, defaults to None"
    )
    parser.add_argument(
        "--mode",
        choices=[
...
def main():
    ...
    if params["mode"]  == "train":
        ...
        est.evaluate(input_fn=simulation_tf_record_eval_input_fn, steps=params["eval_steps"], name='validation')
    else:
        est.compile(
            train_input_fn,
